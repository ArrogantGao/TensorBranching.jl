...
"""
function cover(sub_covers::AbstractVector{SubCover{INT}}, max_itr::Int, ::LPSetCoverSolver; min_complexity::TF = 1.0) where{INT, TF}
    cx, n = γ0(sub_covers)
    ...
    for i =1:max_itr
        ...
        @debug "LP Solver, Iteration $i: picked_num = $(length(picked)), complexity = $cx"
...
function cover(sub_covers::AbstractVector{SubCover{INT}}, max_itr::Int, ::IPSetCoverSolver; min_complexity::TF = 1.0) where{INT, TF}
    ...
    for i =1:max_itr
        ...
        cx = complexity(picked)
        @debug "IP Solver, Iteration $i: picked_num = $(length(picked)), complexity = $cx"
        if (cx < min_complexity) || (i == max_itr)
            return picked, cx
        end
    end
...
function IP_setcover(γp::TF, sub_covers::AbstractVector{SubCover{INT}}, n::Int) where{INT, TF}
...
"""
function setcover_strategy(tbl::BranchingTable{INT}, vertices::Vector{Int}, g::SimpleGraph, max_itr::Int, measure::AbstractMeasure, setcover_solver::AbstractSetCoverSolver) where{INT}
    sub_covers = subcovers(tbl, vertices, g, measure)
    cov, cx = cover(sub_covers, max_itr, setcover_solver)
