...
function __slice_bfs(unfinished_slices::Vector{Tuple{SlicedBranch{Int}, ST}}, slicer::AbstractSlicer, size_dict::Dict{Int, Int}, verbose::Int) where ST
...
function slice(g::SimpleGraph, weights::Vector, code::DynamicNestedEinsum, r, slicer::AbstractSlicer, reducer::AbstractReducer; verbose::Int = 0, dirname = nothing)
    branch = SlicedWeightedBranch(g, weights, code, r)
    return slice(branch, slicer, reducer; verbose = verbose, dirname = dirname)
end

function slice(branch::SlicedWeightedBranch, slicer::AbstractSlicer, reducer::AbstractReducer; verbose::Int = 0, dirname = nothing)
    size_dict = uniformsize(uncompress(branch.code), 2)

    # (complexity(branch).sc ≤ slicer.sc_target) && return [branch]

    if slicer.search_order == :dfs
        slices = Vector{SlicedWeightedBranch{Int}}()
        _slice_dfs!(slices, branch, slicer, reducer, size_dict, verbose)
    elseif slicer.search_order == :bfs
        slices = _slice_bfs(branch, slicer, reducer, size_dict, verbose, dirname)
    elseif slicer.search_order == :tree
...
function _slice_dfs!(slices::Vector{SlicedWeightedBranch{Int}}, branch::SlicedWeightedBranch{Int}, slicer::AbstractSlicer, reducer::AbstractReducer, size_dict::Dict{Int, Int}, verbose::Int)
    ...
    uncompressed_code = uncompress(branch.code)
    region, loss = ob_region(branch.g, uncompressed_code, slicer, slicer.region_selector, size_dict, verbose)
    brs = optimal_branches(branch.g, branch.weights, uncompressed_code, branch.r, slicer, reducer, region, size_dict, verbose)
...
function _slice_bfs(branch::SlicedWeightedBranch, slicer::AbstractSlicer, reducer::AbstractReducer, size_dict::Dict{Int, Int}, verbose::Int, dirname)
    ...
    if (complexity(branch).sc ≤ slicer.sc_target)
        ...
        unfinished_slices = [(branch, reducer)]
        finished_slices = Vector{SlicedWeightedBranch{Int}}()
        cc = complexity(branch)
        scs = [cc.sc]
    end
...
function __slice_bfs(unfinished_slices::Vector{Tuple{SlicedWeightedBranch{Int}, ST}}, slicer::AbstractSlicer, size_dict::Dict{Int, Int}, verbose::Int) where ST

    n = length(unfinished_slices)
    temp_slices = Vector{Tuple{SlicedWeightedBranch{Int}, AbstractReducer}}()
    nt = Threads.nthreads()
    chunks = collect(Iterators.partition(1:n, ceil(Int, n/nt)))

    temp_slices = Vector{Vector{Tuple{SlicedWeightedBranch{Int}, AbstractReducer}}}(undef, n)
    ...
    brss = Vector{Tuple{SlicedWeightedBranch{Int}, AbstractReducer}}()
    for i in 1:n
        brs = pop!(temp_slices)
        for (br, reducer) in brs
            push!(brss, (br, reducer))
...
function _slice_bfs_rw(branch::SlicedWeightedBranch, slicer::AbstractSlicer, reducer::AbstractReducer, size_dict::Dict{Int, Int}, verbose::Int, dirname)
    df = CSV.write(joinpath(dirname, "slices.csv"), DataFrame(id = Int[], sc = Float64[], tc = Float64[], r = []))
    ...
    while true
        ...
        Threads.@threads for chunk in chunks
            for i in chunk
                ...
                uncompressed_code = uncompress(new_branch.code)
                region, loss = ob_region(new_branch.g, uncompressed_code, slicer, slicer.region_selector, size_dict, verbose)
                brs = optimal_branches(new_branch.g, new_branch.weights, uncompressed_code, new_branch.r, slicer, new_reducer, region, size_dict, verbose)
...
function slice_tree(g::SimpleGraph, weights::Vector, code::DynamicNestedEinsum, r, slicer::AbstractSlicer, reducer::AbstractReducer; verbose::Int = 0)
    branch = SlicedWeightedBranch(g, weights, code, r)
    size_dict = uniformsize(code, 2)
    return _slice_tree(branch, slicer, reducer, size_dict, verbose)
end

function _slice_tree(branch::SlicedWeightedBranch, slicer::AbstractSlicer, reducer::AbstractReducer, size_dict::Dict{Int, Int}, verbose::Int)
    tree = SlicingWeightedTree(branch, SlicingWeightedTree[])
    if nv(branch.g) == 0 || (complexity(branch).sc ≤ slicer.sc_target) || isnothing(branch.code)
        return tree
    end
