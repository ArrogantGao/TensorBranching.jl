...
function dynamic_ob_mwis(g::SimpleGraph, weights::Vector; reducer::AbstractReducer = TensorNetworkReducer(), slicer::AbstractSlicer = ContractionTreeSlicer(), optimizer::CodeOptimizer = TreeSA(), usecuda::Bool = false, element_type::Type = Float32, verbose::Int = 0)
    # 1. kernelization
    # the first kernelize step is special, leads to tons of reductions
    g_kernelized, weights_kernelized, r, _, _ = kernelize(g, weights, reducer, verbose = verbose)
    (verbose ≥ 1) && (@info "kernelized graph: {$(nv(g_kernelized)), $(ne(g_kernelized))} simple graph")
    nv(g_kernelized) == 0 && return r 
    ...
    return dynamic_ob_mwis(g_kernelized, weights_kernelized, code, r; reducer = reducer, slicer = slicer, usecuda = usecuda, element_type = element_type, verbose = verbose)
end

# if one already has the code of the kernelized graph, then one can directly use this function
function dynamic_ob_mwis(g::SimpleGraph, weights::Vector, code::DynamicNestedEinsum, r; reducer::AbstractReducer = TensorNetworkReducer(), slicer::AbstractSlicer = ContractionTreeSlicer(), usecuda::Bool = false, element_type::Type = Float32, verbose::Int = 0)

    (verbose ≥ 1) && (@info "initial code complexity: \n $(contraction_complexity(code, uniformsize(code, 2)))")

    # 3. slicing via branching, each g_slice should be a tuple of (sliced_graph, contraction_order)
    branches = slice(SlicedWeightedBranch(g, weights, code, r), slicer, reducer, verbose = verbose)
...
function contract_slices(branches::Vector{SlicedWeightedBranch{T1}}, element_type::Type, usecuda::Bool) where {T1}
    res = []
    for branch in branches
        if nv(branch.g) == 0
            push!(res, branch.r)
        else
            net = GenericTensorNetwork(IndependentSet(branch.g, branch.weights),  uncompress(branch.code), Dict{Int, Int}())
            t = Array(solve(net, SizeMax(), usecuda = usecuda))[].n
            res_i = t + branch.r
            push!(res, res_i)
        end
    end
