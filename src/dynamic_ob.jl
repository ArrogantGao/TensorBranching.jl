...
function dynamic_ob_mis(g::SimpleGraph; reducer::AbstractReducer = TensorNetworkReducer(), slicer::AbstractSlicer = ContractionTreeSlicer(), optimizer::CodeOptimizer = TreeSA(), usecuda::Bool = false, use_kernelize::Bool = true, tensor_type::Type = TropicalF32, verbose::Bool = false)
    # 1. kernelization
    if use_kernelize
        g_kernelized, r = kernelize(g, reducer)
        r = Int(r.n)
        verbose && (@info "kernelized graph: {$(nv(g_kernelized)), $(ne(g_kernelized))} simple graph")
    ...
    code = initialize_code(g_kernelized, optimizer, usecuda, tensor_type)
...
function dynamic_ob_mis(g::SimpleGraph, code::DynamicNestedEinsum, r::Int; slicer::AbstractSlicer = ContractionTreeSlicer(), usecuda::Bool = false, tensor_type::Type = TropicalF32, verbose::Bool = false)

    tensors = initialize_tensors(g, usecuda, tensor_type)

    verbose && (@info "initial code complexity: \n $(contraction_complexity(code, uniformsize(code, 2)))")

    # 3. slicing via branching, each g_slice should be a tuple of (sliced_graph, contraction_order)
    branches = slice(SlicedBranch(g, code, r), slicer)

    verbose && (@info "branching result: \n num of branches: $(length(branches)). \n max sc: $(maximum(sc.(branches))). \n total tc: $(log2(sum(2 .^ tc.(branches)))).")
...
function initialize_code(g::SimpleGraph, optimizer::CodeOptimizer, usecuda::Bool, tensor_type::Type)
